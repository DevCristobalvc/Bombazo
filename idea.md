...
- [x] **Multijugador local (2 jugadores, un teléfono)**: modo hot-seat — el pateador remata en secreto, una pantalla de "pásale el teléfono" oculta el tiro, el arquero elige su vuelo a ciegas y recién entonces se revela la animación. Cada uno con su selección; el ganador se anuncia por equipo y no afecta tus estadísticas personales.
...
- [x] **Tiros de esquina (córners)**: modo nuevo con física distinta — el centro barre con comba el área frente al arco (alternando esquina derecha/izquierda) y rematas de cabeza con un toque de timing: la posición del balón al tocar define la columna del remate y su profundidad la altura; tocar muy temprano (balón abierto) suele irse desviado, y si no tocas el centro pasa de largo. Defensa: atajas el cabezazo rival eligiendo casilla. Mismo formato de tanda (5 por lado, corte anticipado, muerte súbita) reutilizando el motor de core/shootout.
- [x] **Tiros libres** con barrera: tercer modo de juego. Tres defensores con el uniforme del equipo que defiende tapan la columna central a media y baja altura — se supera por la fila alta o rodeándola con curva fuerte (|curve| > 0.45); si no, el tiro rebota en el muro (la barrera salta con cada remate). La CPU también puede estrellarse contra tu barrera. Regla pura en `wallBlocks()` de core/physics.js, con test propio.
- [x] **Físicas del personaje y control directo**: el arquero se puede **arrastrar** hasta donde quieres taparlo (además del tap en casilla), su estirada es una parábola física con despegue-vuelo-caída, y el pateador acelera hacia el balón. **Línea de trayectoria** punteada que proyecta el tiro real (con curva) mientras deslizas. **Cámara viva**: acercamientos sutiles según la fase. **Sombreado 3D** con luz de estadio en personajes y balón (menos plano, más trabajado).
- [x] Viento como modificador ligero de la física: cada partido sortea viento (nulo, suave o fuerte, hacia cualquier lado) que empuja lateralmente todos los tiros — indicador en pantalla y el jugador compensa apuntando contra él. Afecta también a la CPU; los duelos online se juegan sin viento para mantener la simetría.
